<script>
  ...
  // console.dir("tutorial content =", card)
</script>
...
<div class="tutorial" use:links>
  ...
  <div class="placeholder-img">lorem</div>
  {#each placeholder.slides as slide, index (slide._key)}
    ...
    {/if}
  {/each}
  {#if placeholder.slides && placeholder.slides.length > 0}
    <div class="navigation">
      ...
      {#if currentIndex != placeholder.slides.length - 1}
      <div
        class="nav-button next"
        class:disabled={currentIndex === placeholder.slides.length - 1}
        on:click={e => {
          currentIndex += 1
        }}>
        Next
